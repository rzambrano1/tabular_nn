...
def encode_categorical(
    df_pl: pl.DataFrame, encode_map: dict[str, dict[str, int]], cat_cols: list[str]
) -> pl.DataFrame:
    """
    ...
    for col_name in cat_cols:
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name).replace(encode_map[col_name]).cast(pl.Int64)
        )
...
def decode_categorical(
    df_pl: pl.DataFrame, decode_map: dict[str, dict[str, int]], cat_cols: list[str]
) -> pl.DataFrame:
    """
    ...
    for col_name in cat_cols:
        ...
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name)
            .map_elements(lambda x, m=curr_map: m.get(x, None), return_dtype=pl.String)  # type: ignore[misc]
            .alias(col_name)
        )
...
def generate_categorical_encoding_mappings(
    df_pl: pl.DataFrame, cat_cols: list[tuple[str, int]], nrow: int
    ...
    for col_name, i in cat_cols:
        unique_vals = (
            df_pl[col_name].unique().sort().to_list()
        )  # Adding sort to make the mappings more deterministic

        if len(unique_vals) > nrow / 3:
            warnings.warn(
                f"Check {col_name} does not contain open ended values. This implementation only process categorical levels..."
            )
...
def generate_categorical_decoding_mappings(
...
def discrete_float_into_int(
    df_pl: pl.DataFrame, columns_to_fix: list[tuple[str, int]]
) -> pl.DataFrame:
    """
...
def generate_numerical_discrete_encoding_mappings(
    df_pl: pl.DataFrame, discrete_cols: list[tuple[str, int]]
...
def generate_numeric_discrete_decoding_mappings(
    encoding_maps: dict[str, dict[int, int]],
) -> dict[str, dict[int, int]]:
    """
...
def encode_numerical_discrete(
    ...
    for col_name in discrete_cols:
        ...
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name).replace(encoding_map[col_name]).cast(pl.Int64)
        )

    return df_pl_encoded


def decode_numerical_discrete(
    df_pl: pl.DataFrame, decode_map: dict[str, dict[int, int]], discrete_cols: list[str]
):
    """
    ...
    for col_name in discrete_cols:
        ...
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name)
            .map_elements(lambda x, m=curr_map: m.get(x, None), return_dtype=pl.Int64)  # type: ignore[misc]
            .alias(col_name)
        )
...
def get_bin_designs(
    ...
    for col_name in cols_to_process_names:
        ...
        edges = np.percentile(
            df_pl[col_name].drop_nulls().to_numpy(), percentiles
...
def generate_numerical_binned_encoding_mappings(
    binned_cols: list[tuple[str, int]], bin_designs: dict[str, BinDesign]
) -> dict[str, dict[tuple[float, float] | None, int]]:
    """
    ...
    for col_name, _ in binned_cols:
        ...
        inner_mapping: dict[tuple[float, float] | None, int] = {}
        ...
        for i in range(curr_n_bins):
            interval = (
                float(curr_edges[i]),
                float(curr_edges[i + 1]),
            )  # Detected by Mypy: need to cast Any into float
            inner_mapping[interval] = (
                i + 1
            )  # The first entry of the dict is None mapped to 0. Thus mapping is shifted by 1
...
def generate_numeric_binned_decoding_mappings(
    encoding_maps: dict[str, dict[tuple[float, float] | None, int]],
...
def find_bin(
    num_to_bin: float, edges: NDArray[np.floating[Any]]
    ...
    while low < high:
        mid = (low + high) // 2
        if (
            edges[mid] <= num_to_bin
        ):  # The binary search uses < but since the interval is open on the right (upper edge) <= is necessary
            low = mid + 1
        else:
            high = mid
...
def encode_numerical_binned(
    ...
    bin_designs: dict[str, BinDesign],
) -> pl.DataFrame:
    """
    ...
    for col_name in binned_cols:
        ...
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name)
            .map_elements(
                lambda x, edges=curr_col_edges, col_map=curr_col_mapping: (  # type: ignore[misc]
                    col_map[None] if x is None else col_map[find_bin(x, edges)]
                ),
...
def decode_numerical_binned(
    ...
    for col_name in binned_cols:
        ...
        df_pl_encoded = df_pl_encoded.with_columns(
            pl.col(col_name)
            .map_elements(
                lambda x, col_map=curr_col_mapping: (  # type: ignore[misc]
                    None
                    if x == 0
...
def pad_numeric_digit_col(
    string_numbers: Sequence[str | None], max_len: int, direction: str
) -> list[str]:
    """
    ...
    if direction not in ["right", "left"]:
        raise ValueError(
            f"The direction parameter only accepts 'right' or 'left values. Client provided {direction}..."
        )
...
def generate_sub_column_values(
    df_pl: pl.DataFrame, col_name: str
    ...
    signs_of_items = [
        (
            "0"
            if pd.isna(item)
            else "0" if not np.isfinite(item) else "0" if item >= 0 else "1"
        )
        for item in list_to_process
    ]

    missing_in_items = [
        "1" if pd.isna(item) else "1" if not np.isfinite(item) else "0"
        for item in list_to_process
    ]
    ...
    decimals_in_numbers = [
        item.rstrip("0") or "0" if item is not None else None
        for item in decimals_in_numbers
    ]

    # Computing length of digits
    len_digits_in_numbers = [
        len(item) if item is not None else 0 for item in digits_in_numbers
    ]
    n_digit_sub_cols = max(len_digits_in_numbers)

    # Computing length of decimals
    len_decimals_in_numbers = [
        len(item) if item is not None else 0 for item in decimals_in_numbers
    ]
    n_decimal_sub_cols = max(len_decimals_in_numbers)

    # Computing number of required sub-columns for the variable being processed
    n_sub_cols = (
        n_digit_sub_cols + n_decimal_sub_cols + 1 + 1
    )  # Adding one for a sub-column to store the sign and another to store NAs

    # Padding digits
    padded_digits = pad_numeric_digit_col(digits_in_numbers, n_digit_sub_cols, "left")

    # Padding decimals
    padded_decimals = pad_numeric_digit_col(
        decimals_in_numbers, n_decimal_sub_cols, "right"
    )

    # Generating the values for all sub-columns as strings
    signs_and_pad_digits = [
        f"{prefix}{value}" for prefix, value in zip(signs_of_items, padded_digits)
    ]
    signs_and_pad_digits_and_decimals = [
        f"{prefix}{value}"
        for prefix, value in zip(signs_and_pad_digits, padded_decimals)
    ]
    sub_column_values_as_string = [
        f"{prefix}{value}"
        for prefix, value in zip(signs_and_pad_digits_and_decimals, missing_in_items)
    ]

    return n_sub_cols, n_digit_sub_cols, n_decimal_sub_cols, sub_column_values_as_string


def encode_numerical_digit(
    df_pl: pl.DataFrame, digit_cols: list[str]
    ...
    for col_name in digit_cols:
        ...
        (
            ...
            n_decimal_sub_cols,
            sub_column_values_as_string,
        ) = generate_sub_column_values(processed_df, col_name)
...
def decode_numerical_digit(
    ...
    for col_name in digit_cols:
        ...
        all_columns_in_encoding = [
            item for item in df_decoded.columns if item.startswith(f"{col_name}_")
        ]  # trying this approach that seems to be more precise
        ...
        digit_columns_in_encoding = all_columns_in_encoding[
            1 : (curr_col_encoding[0] + 1)
        ]
        decimal_columns_in_encoding = all_columns_in_encoding[curr_col_encoding[1] : -1]

        # Joining Digits
        digits_in_magnitude = [
            str(item) for item in df_decoded[digit_columns_in_encoding[0]].to_list()
        ]

        for digit_sub_col in digit_columns_in_encoding[1:]:
            next_digit_list = [
                str(item) for item in df_decoded[digit_sub_col].to_list()
            ]
            digits_in_magnitude = [
                f"{digit}{next_digit}"
                for digit, next_digit in zip(digits_in_magnitude, next_digit_list)
            ]

        # Joining Decimals
        decimals_in_magnitude = [
            str(item) for item in df_decoded[decimal_columns_in_encoding[0]].to_list()
        ]

        for decimal_sub_col in decimal_columns_in_encoding[1:]:
            next_decimal_list = [
                str(item) for item in df_decoded[decimal_sub_col].to_list()
            ]
            decimals_in_magnitude = [
                f"{decimal}{next_decimal}"
                for decimal, next_decimal in zip(
                    decimals_in_magnitude, next_decimal_list
                )
            ]
        ...
        magnitudes_as_str = [
            f"{digits}.{decimals}"
            for digits, decimals in zip(digits_in_magnitude, decimals_in_magnitude)
        ]
...
def encode_datetime(
    df_pl: pl.DataFrame, datetime_cols: list[str]
) -> tuple[pl.DataFrame, dict[str, type[pl.DataType]]]:
    """
    ...
    for col_name in datetime_cols:
        ...
        for valid_component in curr_col_valid_components:
            ...
            component_unique_values = (
                ...
                .dt.__getattribute__(valid_component)()
                .drop_nulls()
                .n_unique()
            )
...
def decode_datetime(
    ...
    datetime_encoding_map: dict[str, type[pl.DataType]],
) -> pl.DataFrame:
    """
    Reconstructs float columns from their digit sub-column encoding.
    ...
    for col_name in datetime_columns:
        ...
        curr_subcol_names = {
            c: pl.col(c) for c in df_decoded.columns if c.startswith(f"{col_name}_")
        }
